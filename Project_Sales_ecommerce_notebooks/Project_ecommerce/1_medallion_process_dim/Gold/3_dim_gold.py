...
df_products = spark.table(f"{catalog_name}.sliver.slv_products")
df_brands = spark.table(f"{catalog_name}.sliver.slv_brands")
df_category = spark.table(f"{catalog_name}.sliver.slv_category")
...
# MAGIC   p._source_file as file_name,
# MAGIC   p.ingested_at as ingest_timestamp
...
df_silver = spark.table(f'{catalog_name}.sliver.slv_customers')
...
df_silver = spark.table(f'{catalog_name}.sliver.slv_calendar')
display(df_silver.limit(5))
